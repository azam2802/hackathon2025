...
### 6. Настройка API (опционально)

Для интеграции с backend API настройте следующие переменные в `.env`:

- `API_BASE_URL` - URL backend сервера
- `API_KEY` - ключ для авторизации API
- `API_ENABLED` - включение/отключение отправки в API (`true`/`false`)
